...
class IndianMarketDataFetcher:
    ...
    def __init__(self, alpha_vantage_api_key: Optional[str] = None, quandl_api_key: Optional[str] = None,
                 upstox_api_key: Optional[str] = None, upstox_access_token: Optional[str] = None):
        ...
        try:
            ...
            self.news_api_key = getattr(config, 'NEWS_API_KEY', "your_news_api_key_here")
            self.finnhub_api_key = getattr(config, 'FINNHUB_API_KEY', "your_finnhub_api_key_here")
            self.polygon_api_key = getattr(config, 'POLYGON_API_KEY', "your_polygon_api_key_here")
            self.upstox_api_key = upstox_api_key or getattr(config, 'UPSTOX_API_KEY', "your_upstox_api_key_here")
            self.upstox_access_token = upstox_access_token or getattr(config, 'UPSTOX_ACCESS_TOKEN', "your_upstox_access_token_here")
            ...
            self.upstox_api_key = upstox_api_key or "your_upstox_api_key_here"
            self.upstox_access_token = upstox_access_token or "your_upstox_access_token_here"
    ...
    def _fetch_yahoo_finance_enhanced(self, symbol: str, period: str, interval: str) -> pd.DataFrame:
        """Enhanced Yahoo Finance data fetching with better error handling"""
        try:
            # Create ticker with enhanced settings
            ticker = yf.Ticker(symbol)
            
            # Configure ticker for better data quality
            ticker.info  # This helps initialize the ticker
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if not data.empty:
                        ...
                        data = data.dropna()
                        data = data[data['Volume'] > 0]  # Remove zero volume data
                        ...
                        logger.info(f"Yahoo Finance: Fetched {len(data)} records for {symbol}")
                        return data
                    else:
                        logger.warning(f"Yahoo Finance: Empty data for {symbol}, attempt {attempt + 1}")
            ...
            logger.error(f"Yahoo Finance enhanced fetch failed: {e}")
            return pd.DataFrame()
    
    def _fetch_upstox_ohlcv(self, symbol: str, period: str, interval: str) -> pd.DataFrame:
        """Fetch OHLCV data from Upstox API"""
        try:
            # Map symbol to Upstox format
            upstox_symbol = self._map_to_upstox_symbol(symbol)
            if not upstox_symbol:
                logger.warning(f"Symbol {symbol} not mapped for Upstox")
                return pd.DataFrame()
            
            # Map period and interval to Upstox format
            upstox_interval = self._map_to_upstox_interval(interval)
            upstox_period = self._map_to_upstox_period(period)
            
            # Upstox API endpoint
            url = f"https://api.upstox.com/v2/historical-candle/{upstox_symbol}/{upstox_interval}/{upstox_period}"
            ...
            if response.status_code == 200:
                ...
                if data.get('status') == 'success' and 'data' in data:
                    ...
                    if candles:
                        ...
                        logger.info(f"Upstox: Fetched {len(df)} records for {symbol}")
                        return df
                    else:
                        logger.warning(f"Upstox: No candle data for {symbol}")
                        return pd.DataFrame()
                else:
                    logger.warning(f"Upstox API error: {data.get('message', 'Unknown error')}")
                    return pd.DataFrame()
            else:
                logger.warning(f"Upstox API HTTP error: {response.status_code}")
                return pd.DataFrame()
                
        except Exception as e:
            logger.error(f"Upstox fetch failed: {e}")
            return pd.DataFrame()
    
    def _map_to_upstox_symbol(self, symbol: str) -> str:
        """Map Indian market symbol to Upstox format"""
        upstox_mapping = {
        ...
        return upstox_mapping.get(symbol, '')
    
    def _map_to_upstox_interval(self, interval: str) -> str:
        ...
        return interval_mapping.get(interval, 'day')
    
    def _map_to_upstox_period(self, period: str) -> str:
    ...
    def _fetch_from_multiple_sources(self, symbol: str, period: str, interval: str) -> pd.DataFrame:
        ...
        # Source 1: Enhanced Yahoo Finance (Primary)
        try:
            logger.info(f"Attempting to fetch {symbol} data from Yahoo Finance...")
            data = self._fetch_yahoo_finance_enhanced(yahoo_symbol, period, interval)
        ...
        # Source 2: Upstox API (if API key available)
        if (self.upstox_api_key and self.upstox_api_key != "your_upstox_api_key_here" and
            self.upstox_access_token and self.upstox_access_token != "your_upstox_access_token_here"):
            try:
                logger.info(f"Attempting to fetch {symbol} data from Upstox...")
                data = self._fetch_upstox_ohlcv(symbol, period, interval)
                if not data.empty:
                    logger.info(f"Successfully fetched {len(data)} records from Upstox")
                    # Cache the data
                    self.data_cache[cache_key] = (data, time.time())
                    return data
            except Exception as e:
                logger.warning(f"Upstox failed: {e}")
